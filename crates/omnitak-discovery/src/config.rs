...
mod tests {
    ...
    fn test_service_type_conversion() {
        let tak = ServiceType::TakServer;
        assert_eq!(tak.to_service_string(), "_tak._tcp.local.");
        assert_eq!(
            ServiceType::from_service_string("_tak._tcp.local"),
            ServiceType::TakServer
        );
    }
    ...
    fn test_config_validation() {
        let valid = DiscoveryConfig::default();
    ...
    fn test_custom_service_type() {
        let custom = ServiceType::Custom("_myservice._tcp".to_string());
        assert_eq!(custom.to_service_string(), "_myservice._tcp.local.");
    }
}
